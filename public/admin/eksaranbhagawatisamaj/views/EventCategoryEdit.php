...
// Page object
$EventCategoryEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { event_category: currentTable } });
var currentForm, currentPageID;
var fevent_categoryedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fevent_categoryedit = new ew.Form("fevent_categoryedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fevent_categoryedit;

    // Add fields
    var fields = currentTable.fields;
    fevent_categoryedit.addFields([
    ...
    // Form_CustomValidate
    fevent_categoryedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fevent_categoryedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fevent_categoryedit.lists.Active = <?= $Page->Active->toClientList($Page) ?>;
    loadjs.done("fevent_categoryedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fevent_categoryedit" id="fevent_categoryedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->ID->Visible) { // ID ?>
    <div id="r_ID"<?= $Page->ID->rowAttributes() ?>>
        <label id="elh_event_category_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->ID->caption() ?><?= $Page->ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->ID->cellAttributes() ?>>
<span id="el_event_category_ID">
<span<?= $Page->ID->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->ID->getDisplayValue($Page->ID->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="event_category" data-field="x_ID" data-hidden="1" name="x_ID" id="x_ID" value="<?= HtmlEncode($Page->ID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
