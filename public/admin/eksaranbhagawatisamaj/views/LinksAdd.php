...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    flinksadd = new ew.Form("flinksadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = flinksadd;
    ...
    // Form_CustomValidate
    flinksadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    flinksadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    flinksadd.lists.Active = <?= $Page->Active->toClientList($Page) ?>;
    loadjs.done("flinksadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="flinksadd" id="flinksadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<script>
loadjs.ready(["flinksadd", "editor"], function() {
	ew.createEditor("flinksadd", "x_Link_Content", 35, 4, <?= $Page->Link_Content->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
