...
// Page object
$SiteSettingsEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { site_settings: currentTable } });
var currentForm, currentPageID;
var fsite_settingsedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fsite_settingsedit = new ew.Form("fsite_settingsedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fsite_settingsedit;

    // Add fields
    var fields = currentTable.fields;
    fsite_settingsedit.addFields([
    ...
    // Form_CustomValidate
    fsite_settingsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fsite_settingsedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fsite_settingsedit.lists.Active = <?= $Page->Active->toClientList($Page) ?>;
    loadjs.done("fsite_settingsedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fsite_settingsedit" id="fsite_settingsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->ID->Visible) { // ID ?>
    <div id="r_ID"<?= $Page->ID->rowAttributes() ?>>
        <label id="elh_site_settings_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->ID->caption() ?><?= $Page->ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->ID->cellAttributes() ?>>
<span id="el_site_settings_ID">
<span<?= $Page->ID->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->ID->getDisplayValue($Page->ID->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="site_settings" data-field="x_ID" data-hidden="1" name="x_ID" id="x_ID" value="<?= HtmlEncode($Page->ID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" name="fa_x_Favicon" id= "fa_x_Favicon" value="<?= (Post("fa_x_Favicon") == "0") ? "0" : "1" ?>">
