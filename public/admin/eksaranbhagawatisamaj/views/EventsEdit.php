...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    feventsedit = new ew.Form("feventsedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = feventsedit;

    // Add fields
    var fields = currentTable.fields;
    feventsedit.addFields([
    ...
    // Form_CustomValidate
    feventsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    feventsedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    feventsedit.lists.Event_Category = <?= $Page->Event_Category->toClientList($Page) ?>;
    feventsedit.lists.Active = <?= $Page->Active->toClientList($Page) ?>;
    loadjs.done("feventsedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="feventsedit" id="feventsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->ID->Visible) { // ID ?>
    <div id="r_ID"<?= $Page->ID->rowAttributes() ?>>
        <label id="elh_events_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->ID->caption() ?><?= $Page->ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->ID->cellAttributes() ?>>
<span id="el_events_ID">
<span<?= $Page->ID->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->ID->getDisplayValue($Page->ID->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="events" data-field="x_ID" data-hidden="1" name="x_ID" id="x_ID" value="<?= HtmlEncode($Page->ID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["feventsedit", "datetimepicker"], function () {
    let options = {
        localization: {
    ...
    ew.createDateTimePicker("feventsedit", "x_Event_Date", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_events_Event_Category">
    <select
        ...
        data-select2-id="feventsedit_x_Event_Category"
        data-table="events"
...
<script>
loadjs.ready("feventsedit", function() {
    var options = { name: "x_Event_Category", selectId: "feventsedit_x_Event_Category" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (feventsedit.lists.Event_Category.lookupOptions.length) {
        options.data = { id: "x_Event_Category", form: "feventsedit" };
    } else {
        options.ajax = { id: "x_Event_Category", form: "feventsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["feventsedit", "editor"], function() {
	ew.createEditor("feventsedit", "x__Content", 35, 4, <?= $Page->_Content->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
