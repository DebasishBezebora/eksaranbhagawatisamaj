...
class Events extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // Event_Date
        $this->Event_Date = new DbField(
            'events',
            'events',
            'x_Event_Date',
            'Event_Date',
            '`Event_Date`',
            CastDateFieldForLike("`Event_Date`", 0, "DB"),
            133,
            10,
            0,
            false,
        ...
        $this->Event_Date->InputTextType = "text";
        ...
        $this->Event_Category->Nullable = false; // NOT NULL field
        $this->Event_Category->Required = true; // Required field
        $this->Event_Category->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->Event_Category->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        $this->Event_Category->Lookup = new Lookup('Event_Category', 'event_category', false, 'ID', ["Category","","",""], [], [], [], [], [], [], '`ID` DESC', '', "`Category`");
        $this->Event_Category->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        ...
        $this->Active->Lookup = new Lookup('Active', 'events', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("EventsView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("EventsView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "EventsAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "EventsAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("EventsEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("EventsAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("EventsDelete", $this->getUrlParm());
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->Event_Date->EditValue = FormatDateTime($this->Event_Date->CurrentValue, $this->Event_Date->formatPattern());
